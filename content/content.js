(() => {
  ...
  function getHoverReadingTarget(x, y) {
    ...
    if (isGoogleDocs()) {
      if (!isInGoogleDocsEditor(el)) return null;
  ...
  async function startReadingFromTarget(target) {
    if (!target) return;
  ...
  function showGoogleDocsHint(message) {
    ...
    hint.textContent = message || "Hover 2 giây trên dòng để hiện ▶, hoặc double-click để đọc";
  ...
  if (isGoogleDocs()) {
    showGoogleDocsHint();
    setTimeout(showGoogleDocsHint, 3000);
  }
})();
